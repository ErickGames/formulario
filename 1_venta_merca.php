<?php
include_once('header.php');

session_start();

?>

<!DOCTYPE html>
<html lang="en">

<body>

    <div class="mx-5 mt-5">
        <h4>SECCION 1</h4>
        <h1>VENTAS Y MERCADOTECNIA</h1>
    </div>

    <form action="2_recepciones.php" method="POST" class="m-5">

        <!-- PREGUNTA 1 -->
        <div class="p-5" style="background-color:rgba(25, 47, 89, 0.1)">

            <h4 class="mb-3 ">Gestión de venta</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p1" value="s1p1r1" required>
                <label class="form-check-label">
                    No se cuenta con una labor de venta específica, solo la atención de clientes que nos buscan.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p1" value="s1p1r2" required>
                <label class="form-check-label">
                    Se cuenta con los mínimos protocolos de venta, para conectar con clientes de manera presencial y
                    virtual.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p1" value="s1p1r3" required>
                <label class="form-check-label">
                    Solo se cuenta con seguimiento manual.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p1" value="s1p1r4" required>
                <label class="form-check-label">
                    Se cuenta con un CRM para la integración y monitorio de los pacientes actuales y posibles nuevos.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p1" value="s1p1r5" required>
                <label class="form-check-label">
                    Se cuenta con un CRM que es explotado y aprovechado al máximo nivel.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 2 -->
        <div class="p-5" style="background-color:rgba(61, 177, 102, 0.1)">

            <h4 Class="mb-3 ">Actividad Promocional</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p2" value="s1p2r1">
                <label class="form-check-label">
                    No se cuenta con actividad promocional.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p2" value="s1p2r2">
                <label class="form-check-label">
                    Se cuenta con promoción solo en algunas fechas importantes.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p2" value="s1p2r3">
                <label class="form-check-label">
                    Se cuenta con promoción fechas importantes y en servicios específicos a lo largo del año.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p2" value="s1p2r4">
                <label class="form-check-label">
                    Se cuenta con un programa anual de promociones aprovechando las temporalidades de año.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p2" value="s1p2r5">
                <label class="form-check-label">
                    Se cuenta con un programa anual de promociones además de un plan de lealtad con privilegios
                    exclusivos.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 3 -->
        <div class="p-5" style="background-color:rgba(25, 47, 89, 0.1)">


            <h4 class="mb-3 ">Gestión de venta</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p3" value="s1p3r1">
                <label class="form-check-label">
                    No se cuenta con una labor de venta específica, solo la atención de clientes que nos buscan.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p3" value="s1p3r2">
                <label class="form-check-label">
                    Se cuenta con los mínimos protocolos de venta, para conectar con clientes de manera presencial y
                    virtual.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p3" value="s1p3r3">
                <label class="form-check-label">
                    Solo se cuenta con seguimiento manual.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p3" value="s1p3r4">
                <label class="form-check-label">
                    Se cuenta con un CRM para la integración y monitorio de los pacientes actuales y posibles nuevos.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p3" value="s1p3r5">
                <label class="form-check-label">
                    Se cuenta con un CRM que es explotado y aprovechado al máximo nivel.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 4 -->
        <div class="p-5" style="background-color:rgba(61, 177, 102, 0.1)">

            <h4 class="mb-3 ">Redes Sociales</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p4" value="s1p4r1">
                <label class="form-check-label">
                    No se cuenta con redes sociales.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p4" value="s1p4r2">
                <label class="form-check-label">
                    Se cuenta con Facebook (Solo se postea o sube contenido aislado sin estrategia).
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p4" value="s1p4r3">
                <label class="form-check-label">
                    Se cuenta con Facebook / Instagram.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p4" value="s1p4r4">
                <label class="form-check-label">
                    Se cuenta con Facebook / Instagram / Tik-Tok y tienen estrategia de contenido.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p4" value="s1p4r5">
                <label class="form-check-label">
                    Se tiene y ejecuta una estrategia efectiva y rentable que genera venta y utilidad incremental.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 5 -->
        <div class="p-5" style="background-color:rgba(25, 47, 89, 0.1)">

            <h4 Class="mb-3 ">Poder de marca</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p5" value="s1p5r1">
                <label class="form-check-label">
                    No se cuenta con una marca.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p5" value="s1p5r2">
                <label class="form-check-label">
                    Se cuenta con una marca básica y con oportunidades para comunicación.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p5" value="s1p5r3">
                <label class="form-check-label">
                    Se tiene una marca bien definida pero poco conocida.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p5" value="s1p5r4">
                <label class="form-check-label">
                    Se cuenta con una buena marca, muy reconocida pero no se explotada para generar venta incremental.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p5" value="s1p5r5">
                <label class="form-check-label">
                    Se cuenta con una marca muy reconocida y explotada en una medida generando venta orgánica sin
                    necesidad de pago en comunicación.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 6 -->
        <div class="p-5" style="background-color:rgba(61, 177, 102, 0.1)">

            <h4 class="mb-3 ">Página WEB del negocio</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p6" value="s1p6r1">
                <label class="form-check-label">
                    No cuenta con web.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p6" value="s1p6r2">
                <label class="form-check-label">
                    Se tiene una pagina WEB muy básica y sin actualización.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p6" value="s1p6r3">
                <label class="form-check-label">
                    Tiene un web funcional pero muy básica.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p6" value="s1p6r4">
                <label class="form-check-label">
                    Se cuenta con una pagina WEB, funcional y atractiva.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p6" value="s1p6r5">
                <label class="form-check-label">
                    Se cuenta con una pagina WEB, funcional, atractiva y muy efectiva para conectar con los clientes y
                    generar venta incremental.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 7 -->
        <div class="p-5" style="background-color:rgba(25, 47, 89, 0.1)">

            <h4 class="mb-3 ">Publicidad en medios tradicionales </h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p7" value="s1p7r1">
                <label class="form-check-label">
                    No cuenta con publicidad en medios tradicionales.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p7" value="s1p7r2">
                <label class="form-check-label">
                    Se cuenta con publicidad básica en revista y volantes de la zona.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p7" value="s1p7r3">
                <label class="form-check-label">
                    Se cuenta con publicidad en periódicos, revistas y volantes con una efectividad aceptable.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p7" value="s1p7r4">
                <label class="form-check-label">
                    Se cuenta con publicidad tradicional efectiva y rentable además de radio.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p7" value="s1p7r5">
                <label class="form-check-label">
                    Publicaciones efectivas en medios impresos, radio y televisión que impactan directamente en venta
                    incremental.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 8 -->
        <div class="p-5" style="background-color:rgba(61, 177, 102, 0.1)">

            <h4 Class="mb-3 ">Plan de Lealtad e incentivos con pacientes frecuentes</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p8" value="s1p8r1">
                <label class="form-check-label">
                    No se cuenta con un plan de incentivos.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p8" value="s1p8r2">
                <label class="form-check-label">
                    Se tienen incentivos ocasionales para clientes frecuentes.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p8" value="s1p8r3">
                <label class="form-check-label">
                    Se cuenta con un programa de lealtad integral y de incentivos pero manual.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p8" value="s1p8r4">
                <label class="form-check-label">
                    Se cuenta con un programa integral de Lealtad y programa de incentivos que generan venta y utilidad
                    incremental al activarlo.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p8" value="s1p8r5">
                <label class="form-check-label">
                    Tienen un sistema automatizado de lealtad con alto aprovechamiento de parte de los clientes.
                </label>
            </div>
        </div>

        <!-- PREGUNTA 9 -->
        <div class="p-5" style="background-color:rgba(25, 47, 89, 0.1)">


            <h4 class="mb-3 ">Soporte para comunicación y mercadotecnia (Agencia o profesionales)</h4>

            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p9" value="s1p9r1">
                <label class="form-check-label">
                    No se cuenta con equipo externo ni interno para impulsar una estrategia de comunicación y
                    mercadotecnia.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p9" value="s1p9r2">
                <label class="form-check-label">
                    Existe una persona que internamente hace labores de marketing de manera parcial.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p9" value="s1p9r3">
                <label class="form-check-label">
                    Existe un recurso interno y también se cuenta con apoyo externo con alcance limitado.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p9" value="s1p9r4">
                <label class="form-check-label">
                    Se cuenta con apoyo externo, experto y efectivo en el diseño y ejecución de contenidos.
                </label>
            </div>
            <div class="form-check mx-4">
                <input class="form-check-input" type="radio" name="s1p9" value="s1p9r5">
                <label class="form-check-label">
                    Se cuenta con apoyo externo, experto y efectivo en el diseño y ejecución de contenidos, además de
                    recurso interno.
                </label>
            </div>
        </div>

        <div style="text-align:center">
            <button id="miBoton" class="cssbuttons-io-button" type="submit">
                Siguiente
                <div class="icon">
                    <svg height="24" width="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 0h24v24H0z" fill="none"></path>
                        <path d="M16.172 11l-5.364-5.364 1.414-1.414L20 12l-7.778 7.778-1.414-1.414L16.172 13H4v-2z"
                            fill="currentColor"></path>
                    </svg>
                </div>
            </button>
        </div>
    </form>


</body>

</html>

<?php
include('footer.php');
?>